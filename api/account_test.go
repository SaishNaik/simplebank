...
func TestCreateAccountAPI(t *testing.T) {
	...
	tc := []struct {
		...
		{
			name: "some error from database not related to unique_violation or foreign_key_violation",
			body: gin.H{
				"currency": account.Currency,
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
		...
		{
			name: "error from database related to unique_violation",
			body: gin.H{
				"currency": account.Currency,
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
		...
		{
			name: "error from database related to foreign_key_violation",
			body: gin.H{
				"currency": account.Currency,
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
	...
	for i := range tc {
		t.Run(tc[i].name, func(t *testing.T) {
			...
			data, err := json.Marshal(tc[i].body)
			require.NoError(t, err)
			...
			tc[i].setupAuth(t, req, server.tokenMaker)
